# Validation — *Patrologia Graeca* 98 (Germain / Tarase)

## Décision : READY — volume entier, images seulement

* **Source/droits :** [Commons, *Patrologia Graeca* 98](https://commons.wikimedia.org/wiki/File:Patrologia_Graeca_Vol._098.pdf), Migne MPG 98, licence `Public domain` selon l'API Commons.
* **Fichier :** PDF image-only, 103 744 495 octets, SHA1 `c62fc965421e011132098457399a17358ec654df`, 832 pages, rendus 960 × ~1505 px (~145 ppi). Absent du registre et de `scans/`.
* **Qualité :** p.20 cols.31–32, p.400 cols.711–712, p.750 cols.1337–1338 : grec net et lisible. Les vues temporaires ont été supprimées ; aucun OCR/image/archive local.

## Mapping direct (9 notices)

| Notices TLG | Colonnes MPG 98 |
|---|---:|
| 2933.003–005 | 40–88 ; 147–221 ; 221–381 |
| 2938.001 | 221–289 |
| 3416.001 ; 9052.007–009 | 1265–1269 ; 1333–1360 ; 1360–1364 ; 1364–1368 |
| 3119.001 | 1481–1500 |

Les autres notices candidates ne donnent pas de locus MPG98 dans le canon. Dédupliquer les frontières communes (221 et 1360/1364). Rendement READY : 9 notices exactes, droits explicites et qualité visuelle validée.

## Compaction HF vérifiée

- PDF image-only original transféré sous `scans/volumes/pg98-germanus-tarasius/pg98-original-image-only.pdf` : 103 744 495 octets, 832 pages.
- SHA-256 local et distant : `4d8c8c698fb095a7dbf60ad25dc3a7863a281490074bc9140b3bbfd3a8d91b3c`.
- Le tar redondant de 609 546 240 octets a été supprimé après vérification distante ; récupération possible via l'historique HF.
