# Validation — *Patrologia Graeca* 114 (hagiographie / Symeon Metaphrastes)

## Décision : READY_PARTIAL — volume ouvert, avec exclusion qualité localisée

* **Édition/droits :** [Commons, *Patrologia Graeca* 114](https://commons.wikimedia.org/wiki/File:Patrologia_Graeca_Vol._114.pdf), Migne MPG 114, annoncé `Public domain` par l'API Commons.
* **Objet distant :** PDF image-only, 90 347 679 octets, SHA1 `24171c3cedf3fca56a856cb6fa715c41f9584f10`, 768 pages, rendus 960 × ~1500 px (environ 145 ppi). Identifiant absent de `data/scan_sources.csv` et des répertoires `scans/` lors du contrôle.
* **Vérification visuelle, sans OCR :** p. 20 cols. 27–28 (net) ; p. 400 cols. 773–774 et début du *Martyrium Anastasi Persae* (net) ; p. 740 cols. 1417–1418 (net). Ces échantillons ont été supprimés. Un quatrième contrôle ciblé p. 700 cols. 1357–1358 révèle une obstruction/arrachement couvrant une forte partie inférieure : ne pas déclarer ces colonnes exploitables.

## Mappings directs sûrs (26 notices)

| Notices TLG | Colonnes MPG 114 |
|---|---:|
| 3115.026, .029, .037, .040 | 1001–1009 ; 968–981 ; 816–893 ; 336–392 |
| 3115.049, .056, .067 | 1293–1312 ; 305–321 ; 397–416 |
| 3115.082, .091 | 896–965 ; 1437–1452 |
| 3115.101–110 | 417–429 ; 429–456 ; 469–553 ; 553–565 ; 761–773 ; 981–1000 ; 1014–1043 ; 1253–1268 ; 1232–1249 ; 1312–1328 |
| 3115.116–117 | 133–134 ; 133 |
| 5123.001 ; 5239.001 ; 5291.001 ; 5296.001 ; 5474.001 | 1377–1436 ; 1332–1345 ; 568–581 ; 1368–1376 ; 1213–1224 |

### Exclure du transfert ciblé

| Notice | Colonnes | Motif |
|---|---:|---|
| 5295.001 | 1348–1365 | le feuillet cols. 1357–1358 est matériellement masqué ; la plage est incomplète dans cette source. |

Le volume peut donc être ingéré pour les 26 notices ci-dessus, sélectionné par **colonnes imprimées**, sans la plage 1348–1365. Toute vue temporaire (quatre JPEG) a été supprimée, et aucun OCR/image/archive n'a été conservé localement.

## Compaction HF vérifiée

- PDF image-only original transféré sous `scans/volumes/pg114-symeon-metaphrastes/pg114-original-image-only.pdf` : 90 347 679 octets, 768 pages.
- SHA-256 local et distant : `f926b79c3a89a18aed1e4a1018dac8f6c889d800e5fa7e150ffdeebf4cc7aa80`.
- Le tar redondant de 602 255 360 octets a été supprimé après vérification distante. L'exclusion de 5295.001 reste obligatoire ; le PDF compact conserve la même obstruction matérielle.
