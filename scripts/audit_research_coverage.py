...
out=[]
for r in canon:
 q=(r['tlg_author_id'],r['tlg_work_id']); c='REVIEWED_EXPLICIT' if e[q] else ('REVIEWED_RANGE_INFERRED' if i[q] else 'UNREVIEWED');out.append([*q,r['author_heading'],r['work_title'],e[q],'; '.join(sorted(fs[q])),'; '.join(sorted(ss[q])),'; '.join(sorted(i[q])),c])
with open(d/'research_coverage_audit.csv','w',newline='') as h:w=csv.writer(h);w.writerow(['tlg_author_id','tlg_work_id','author','work_title','journal_rows','journal_files','statuses','inferred_range_files','coverage']);w.writerows(out)
