import csv
w=list(csv.DictReader(open('data/canon_works.csv')));v=list(csv.DictReader(open('data/text_verification.csv')));m={(x['tlg_author_id'],x['tlg_work_id']):x for x in v};ids={('5521','003'),('9517','001'),('3289','001'),('3289','002'),('3048','x01')};h='tlg_author_id tlg_work_id author_heading work_title canonical_edition sources_tested open_text_result open_text_url open_text_license scan_result scan_url scan_rights confidence proposed_status next_action notes last_checked'.split()
with open('data/research_batches/detailed_verified_3001_batch_88.csv','w',newline='') as f:
 ...
 for q in w:
  ...
  if k[1].startswith('x'):x.update(sources_tested='Canon cross-reference checked',open_text_result='No independent acquisition.',confidence='high',proposed_status='CROSS_REFERENCE',next_action='Resolve cited host; do not duplicate.',notes='Cross-reference retained.')
   ...
   y=m.get(k);x.update(sources_tested='First1KGreek/Perseus exact TLG ID; external catalogue check',scan_result='No edition-matching reusable scan verified.',next_action='Compare against cited edition; no OCR.',notes='No provider OCR used.')
