"""Verified register for five rows after 0198.001, resolving Anthology cross-reference."""
import csv
START=("0198","002");OUTPUT="data/research_batches/detailed_verified_3001_batch_135.csv"
...
with open(OUTPUT,"w",newline="",encoding="utf-8") as fh:
 ...
 for w in rows:
  ...
  if key==("0198","x01"):
   r.update(sources_tested="Canon cross-reference resolved to 7052.001--005 (Anthologiae Graecae Appendix).",open_text_result="No independent acquisition: host works control text.",scan_result="No separate scan candidate: cross-reference only.",confidence="high",proposed_status="CROSS_REFERENCE",next_action="Process hosts 7052.001--005; do not duplicate this referring record.",notes="Resolved canon reference; no image acquired.")
  else:
   r.update(sources_tested="Exact TLG/First1K lookup; cited fragment/Anthologia catalogue check",open_text_result="No exact licensed TEI match in local corpus.",scan_result="No edition-matching reusable page-image source verified.",confidence="medium",proposed_status="EDITION_IDENTIFIED",next_action="Locate a rights-cleared scan of the cited edition; do not substitute OCR.",notes="No image acquired.")
  out.writerow(r)
