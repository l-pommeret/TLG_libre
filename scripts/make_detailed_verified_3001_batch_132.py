"""Verified register for five canonical rows from 1389.001; no image/OCR download."""
import csv
START=("1389","001");OUTPUT="data/research_batches/detailed_verified_3001_batch_132.csv"
...
with open(OUTPUT,"w",newline="",encoding="utf-8") as fh:
 ...
 for w in rows:
  ...
  if key==("1389","002"):
   r.update(sources_tested="Canon duplicate relation checked: 1389.002 duplicates 1389.001 in Keaney 1991.",open_text_result="No independent acquisition: 1389.001 controls shared lexicon text.",scan_result="No separate scan candidate: duplicate edition record.",confidence="high",proposed_status="DUPLICATE_EDITION_REFERENCE",next_action="Use 1389.001 as text host; retain Keaney 1991 solely as bibliographic reference.",notes="No duplicate text/image acquisition.")
  elif key==("0913","x01"):
   r.update(sources_tested="Canon cross-reference resolved to 0057.076 (Galenus, Med.).",open_text_result="No independent acquisition: host work controls text.",scan_result="No separate scan candidate: cross-reference only.",confidence="high",proposed_status="CROSS_REFERENCE",next_action="Process host 0057.076; do not duplicate this referring record.",notes="Resolved canon reference; no image acquired.")
  else:
   r.update(sources_tested="Exact TLG/First1K lookup; cited edition/Anthologia catalogue check",open_text_result="No exact licensed TEI match in local corpus.",scan_result="No edition-matching reusable page-image source verified.",confidence="medium",proposed_status="EDITION_IDENTIFIED",next_action="Locate a rights-cleared scan of the cited edition; do not substitute OCR.",notes="No image acquired.")
  out.writerow(r)
