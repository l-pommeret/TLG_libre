"""Verified register for five canonical rows beginning at 0283.002."""
import csv

START = ("0283", "002")
OUTPUT = "data/research_batches/detailed_verified_3001_batch_156.csv"
FIELDS = (
...
assert len(works) == 5
assert [(w["tlg_author_id"], w["tlg_work_id"]) for w in works] == [
    ("0283", "002"), ("1411", "001"), ("1412", "001"), ("1412", "001"), ("1412", "002")
]
...
with open(OUTPUT, "w", newline="", encoding="utf-8") as handle:
    ...
    for work in works:
        ...
        if key == ("0283", "002"):
            row.update(
                sources_tested="Canon partial-duplication relation checked against 0283.001 (fragments 1–2 / 480–481).",
                ...
                next_action="Compare fragments with 0283.001; retain only non-overlapping material; no OCR.",
                notes="Partial duplication must not be treated as a full cross-reference.",
            )
        else:
            row.update(
                sources_tested="Exact TLG/First1K lookup; cited Hercher, Denis, Kock, or Meineke bibliographic record checked.",
                ...
                notes="The two distinct 1412.001 Canon rows are retained separately; no image acquired.",
            )
        writer.writerow(row)
