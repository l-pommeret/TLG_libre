"""Verified register for five rows from 0539.001, preserving partial duplication."""
import csv
START=("0539","001");OUTPUT="data/research_batches/detailed_verified_3001_batch_145.csv"
...
with open(OUTPUT,"w",newline="",encoding="utf-8") as fh:
 ...
 for w in rows:
  ...
  if key==("0539","004"):
   r.update(sources_tested="Canon partial-duplication relation checked: 0539.004 overlaps 0539.002 in FHG 1.",open_text_result="Partial overlap: no acquisition decision until fragment-by-fragment comparison.",scan_result="No separate scan candidate confirmed.",confidence="medium",proposed_status="PARTIAL_DUPLICATION_REVIEW",next_action="Compare FHG fragments with 0539.002; retain only non-overlapping material; no OCR.",notes="Partial duplication must not be treated as a full cross-reference.")
  else:
   r.update(sources_tested="Exact TLG/First1K lookup; cited FGrH/papyrological catalogue check",open_text_result="No exact licensed TEI match in local corpus.",scan_result="No edition-matching reusable page-image source verified.",confidence="medium",proposed_status="EDITION_IDENTIFIED",next_action="Locate a rights-cleared scan of the cited edition or papyrus; do not substitute OCR.",notes="No image acquired.")
  out.writerow(r)
