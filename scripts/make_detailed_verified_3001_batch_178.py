"""Verified register for five canonical rows beginning at 0087.002."""
import csv

START = ("0087", "002")
OUTPUT = "data/research_batches/detailed_verified_3001_batch_178.csv"
FIELDS = (
...
verification = {
    ...
    for row in csv.DictReader(open("data/text_verification.csv", encoding="utf-8"))
}
...
with open(OUTPUT, "w", newline="", encoding="utf-8") as handle:
    ...
    for work in works:
        ...
        tei = verification.get(key)
        if tei:
            row.update(
                sources_tested=f"Exact First1KGreek TLG identifier {tei['cts_urn']}; local TEI metadata and CC BY-SA 4.0 declaration checked.",
                ...
                open_text_url=tei["text_url"], open_text_license="CC BY-SA 4.0 (First1KGreek TEI metadata)",
                scan_result="No edition-matching reusable page-image source verified.",
                confidence="high", proposed_status="TEXT_OPEN_UNVERIFIED",
                next_action="Compare Canon boundaries and Lentz edition before ingest; no OCR.",
                notes="Exact identifier, provenance, and TEI licence recorded; no image acquired.",
            )
        else:
            row.update(
                sources_tested="Exact TLG/First1K lookup; cited Lentz bibliographic record checked.",
