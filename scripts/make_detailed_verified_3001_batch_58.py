import csv
w=list(csv.DictReader(open('data/canon_works.csv')));v=list(csv.DictReader(open('data/text_verification.csv')));m={(x['tlg_author_id'],x['tlg_work_id']):x for x in v};ids={('3368','001'),('4413','002'),('4413','003'),('4413','004'),('4413','005')};h='tlg_author_id tlg_work_id author_heading work_title canonical_edition sources_tested open_text_result open_text_url open_text_license scan_result scan_url scan_rights confidence proposed_status next_action notes last_checked'.split()
with open('data/research_batches/detailed_verified_3001_batch_58.csv','w',newline='') as f:
