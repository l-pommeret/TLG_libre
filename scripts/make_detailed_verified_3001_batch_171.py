"""Verified register for five canonical rows beginning at 1422.001."""
import csv

START = ("1422", "001")
OUTPUT = "data/research_batches/detailed_verified_3001_batch_171.csv"
FIELDS = (
...
RELATED = {
    ("1423", "001"): "0114.001 (Antipater, Epigr.)",
    ("0095", "002"): "1595.602 (Philodemus, Phil.)",
}
...
with open(OUTPUT, "w", newline="", encoding="utf-8") as handle:
    ...
    for work in works:
        ...
        if key in RELATED:
            host = RELATED[key]
            row.update(
                sources_tested=f"Canon related/partial-duplication relation checked against {host}.",
                open_text_result="Potentially overlapping material: no acquisition decision until comparison.",
                scan_result="No separate scan candidate confirmed.",
                confidence="medium", proposed_status="PARTIAL_DUPLICATION_REVIEW",
                next_action=f"Compare the cited locus with {host}; retain only non-overlapping text; no OCR.",
                notes="The Canon relation is not converted to a full cross-reference; no image acquired.",
            )
        else:
            row.update(
                sources_tested="Exact TLG/First1K lookup; cited AG, Parente, or FGrH bibliographic record checked.",
