"""Verified register for five canonical rows beginning at 0919.x01."""
import csv

START = ("0919", "x01")
OUTPUT = "data/research_batches/detailed_verified_3001_batch_168.csv"
FIELDS = (
...
with open(OUTPUT, "w", newline="", encoding="utf-8") as handle:
    ...
    for work in works:
        ...
        if key == ("0919", "x01"):
            row.update(
                sources_tested="Canon cross-reference resolved to 0057.076 (Galenus, Med.).",
                ...
                next_action="Process host 0057.076 (Galenus, Med.); do not duplicate this referring record.",
                notes="Resolved canon reference; no image acquired.",
            )
        elif key == ("2317", "001"):
            row.update(
                sources_tested="Canon exact-duplication statement checked against 2317.002 (same Phaedrum scholia).",
                ...
                next_action="Compare the 1901 and 2012 editions against 2317.002; do not duplicate text; no OCR.",
                notes="Canon marks the work as duplicate, not merely related.",
            )
        else:
            row.update(
                sources_tested="Exact TLG/First1K lookup; cited Lucarini-Moreschini, Supplementum Hellenisticum, or Schmidt bibliographic record checked.",
