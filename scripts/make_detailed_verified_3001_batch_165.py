"""Verified register for five canonical rows beginning at 3381.001."""
import csv

START = ("3381", "001")
OUTPUT = "data/research_batches/detailed_verified_3001_batch_165.csv"
FIELDS = (
...
REVIEW_KEY = ("4325", "x01")
RELATED = "1379.013 (Fragmenta Alchemica), 1286 (Corpus Hermeticum), and 4350.129/.142/.167/.168/.169 (Astrologica)"
...
with open(OUTPUT, "w", newline="", encoding="utf-8") as handle:
    ...
    for work in works:
        ...
        if key == REVIEW_KEY:
            row.update(
                sources_tested=f"Canon ‘Cf.’ relations checked against {RELATED}.",
                open_text_result="Related witnesses require comparison; no independent acquisition decision.",
                scan_result="No separate scan candidate confirmed.",
                confidence="medium", proposed_status="PARTIAL_DUPLICATION_REVIEW",
                next_action="Compare the cited witnesses and attribution before ingest; retain only non-overlapping text; no OCR.",
                notes="‘Cf.’ relations are not converted to a full cross-reference; no image acquired.",
            )
        else:
            row.update(
                sources_tested="Exact TLG/First1K lookup; cited Dicks, Berthelot-Ruelle, or Powell bibliographic record checked.",
