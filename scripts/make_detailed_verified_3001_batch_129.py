"""Verified register for five canonical rows after batch 128; no image/OCR download."""
import csv
START=('2359','003'); OUTPUT='data/research_batches/detailed_verified_3001_batch_129.csv'
FIELDS=('tlg_author_id tlg_work_id author_heading work_title canonical_edition sources_tested open_text_result open_text_url open_text_license scan_result scan_url scan_rights confidence proposed_status next_action notes last_checked').split()
coverage=list(csv.DictReader(open('data/canon_coverage.csv',encoding='utf-8')));i=next(n for n,r in enumerate(coverage) if (r['tlg_author_id'],r['tlg_work_id'])==START);rows=coverage[i:i+5];assert len(rows)==5
