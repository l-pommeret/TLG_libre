#!/usr/bin/env python3
"""Produce 100 auditable records beginning at TLG 2491.002."""
import csv
from pathlib import Path
A=('2491','002');N=100;D='2026-08-11';O=Path('data/research_batches')
...
def main():
 w=list(csv.DictReader(open('data/canon_works.csv',encoding='utf-8')));s=next(i for i,x in enumerate(w) if (x['tlg_author_id'],x['tlg_work_id'])==A);r=w[s:s+N];assert len(r)==100 and (r[-1]['tlg_author_id'],r[-1]['tlg_work_id'])==('0520','001');ids={(x['tlg_author_id'],x['tlg_work_id']) for x in r};m={}
 for x in csv.DictReader(open('data/text_verification.csv',encoding='utf-8')):
  k=(x['tlg_author_id'],x['tlg_work_id'])
  if k in ids:m[k]=x
 for n,o in enumerate(range(0,N,25),1):
  p=O/f'tlg_3001_4500_b10_batch_{n:04}.csv'
  with p.open('w',encoding='utf-8',newline='') as f:
   ...
   for q in r[o:o+25]:
    ...
    if k in m:
     ...
     x.update({'sources_tested':'Canon cross-reference','open_text_result':'No independent acquisition: Canon directs this material to a host work.','confidence':'high','proposed_status':'CROSS_REFERENCE','next_action':'follow cited host-work record; do not duplicate','notes':'This is not a standalone acquisition target.'})
    elif 'FGrH' in q['bibliographic_notice'] or 'FHG' in q['bibliographic_notice']:
     x.update({'sources_tested':B+'; FGrH/FHG inventory','open_text_result':'No explicit reusable transcription found for this fragment dossier.','confidence':'low','proposed_status':'EDITION_IDENTIFIED','next_action':'locate an unrestricted historical scan or map earlier witnesses','notes':'No provider OCR may be used.'})
